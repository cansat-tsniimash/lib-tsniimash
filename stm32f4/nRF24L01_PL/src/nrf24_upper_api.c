...
int nrf24_pipe_rx_start(uint8_t pipe_no, const nrf24_pipe_config_t * config)
{
	...
	uint8_t dynpd = 0;
	nrf24_read_register(NRF24_REGADDR_DYNPD, &dynpd, 1);
	if (config->payload_size >= 0)
	{
		nrf24_write_register(rx_pw, (uint8_t *)(&config->payload_size), 1);
		dynpd &= ~(1 << pipe_no);
	}
	else
	{
	...
	nrf24_write_register(NRF24_REGADDR_DYNPD, &dynpd, 1);
