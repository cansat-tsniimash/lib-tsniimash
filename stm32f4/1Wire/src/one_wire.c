...
// GPIO порт, на котором расположена наша шина
#define ONEWIRE_GPIO_PORT	GPIOA
// GPIO пин на котором расположена наша шина
#define ONEWIRE_GPIO_PIN	GPIO_PIN_0
...
static void bus_force_down(void)
{
	port_init.Mode = GPIO_MODE_OUTPUT_OD;
	HAL_GPIO_Init(ONEWIRE_GPIO_PORT, &port_init);
}
...
int onewire_init(void)
{
	...
	port_init.Pin = ONEWIRE_GPIO_PIN;
...
int onewire_reset(void)
{
	...
	delay_us(20);
	bool bit = bus_read(); // TODO: добавить оверсамплинг
...
void onewire_write_bit(bool bit)
{
	if (bit)
	{
		bus_force_down();  // кладем шину на пол
		delay_us(10);      // удерживаем от 1 до 15 мкс
		bus_release();     // отпускаем шину
...
bool onewire_read_bit(void)
{
	...
	bool bus_bit = bus_read(); // TODO: Добавить оверсамплинг
...
void onewire_read_rom(uint8_t * rom_buffer)
{
	...
	onewire_read(rom_buffer, 8 * sizeof(uint8_t));
}
...
int ds18b20_set_config(uint8_t alarm_th, uint8_t alarm_tl, ds18b20_resulution_t resolution)
{
	...
	onewire_write(to_write, sizeof(to_write));
...
int ds18b20_read_raw_temperature(uint16_t * raw_temperature, bool * crc_ok)
{
	...
	// Читаем его целиком
	uint8_t buffer[8];
	onewire_read(buffer, sizeof(buffer));
